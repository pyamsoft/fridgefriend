...
internal constructor(
  ...
  init {
    doOnInflate { pie = Pie.fromChart(binding.coreHeroPie) }
  ...
  override fun onLoadImage(
    ...
    return imageLoader.asDrawable().load(icon).into(imageView)
  }
  ...
  private fun handleItems(items: List<FridgeItem>) {
    ...
    val freshData = Pie.Data(value = freshItems.toFloat(), color = Color.parseColor("#00FF00"))

    val consumedData =
        Pie.Data(value = consumedItems.toFloat(), color = Color.parseColor("#0000FF"))

    val spoiledData = Pie.Data(value = spoiledItems.toFloat(), color = Color.parseColor("#FF0000"))
