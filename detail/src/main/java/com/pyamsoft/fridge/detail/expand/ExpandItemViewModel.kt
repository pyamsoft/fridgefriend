...
internal constructor(
  ...
  fun handlePickSimilar(item: FridgeItem) {
    ...
    // was created and the expiration date if it exists. Apply this difference to the current time
    // for this new item to resolve
    // the expiration date
  }

  private fun CoroutineScope.handleRealtimeEvent(event: FridgeItemChangeEvent) =
      when (event) {
  ...
  private fun setFixMessage(message: String) {
    setState { copy(throwable = if (message.isBlank()) null else FixMessageThrowable(message)) }
  }
