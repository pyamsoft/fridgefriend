...
interface FridgeItem : EmptyModel<FridgeItem> {
  ...
  @CheckResult fun entryId(): FridgeEntry.Id

  @CheckResult fun count(): Int

  @CheckResult fun purchaseTime(): Date?

  @CheckResult fun expireTime(): Date?

  @CheckResult fun presence(): Presence

  @CheckResult fun categoryId(): FridgeCategory.Id?

  @CheckResult fun isReal(): Boolean

  @CheckResult fun migrateTo(entryId: FridgeEntry.Id): FridgeItem

  @CheckResult fun count(count: Int): FridgeItem

  @CheckResult fun expireTime(expireTime: Date): FridgeItem

  @CheckResult fun invalidateExpiration(): FridgeItem

  @CheckResult fun purchaseTime(purchaseTime: Date): FridgeItem

  @CheckResult fun invalidatePurchase(): FridgeItem

  @CheckResult fun presence(presence: Presence): FridgeItem

  @CheckResult fun isConsumed(): Boolean

  @CheckResult fun consumptionDate(): Date?

  @CheckResult fun consume(date: Date): FridgeItem

  @CheckResult fun invalidateConsumption(): FridgeItem

  @CheckResult fun isSpoiled(): Boolean

  @CheckResult fun spoiledDate(): Date?

  @CheckResult fun invalidateSpoiled(): FridgeItem

  @CheckResult fun spoil(date: Date): FridgeItem

  @CheckResult fun invalidateCategoryId(): FridgeItem

  @CheckResult fun categoryId(id: FridgeCategory.Id): FridgeItem

  @CheckResult fun makeReal(): FridgeItem
...
fun FridgeItem.isExpired(date: Calendar, countSameDayAsExpired: Boolean): Boolean {
  ...
  // Clean Y/M/D only
  val expiration = today().also { it.time = expireTime }.cleanMidnight()
...
fun FridgeItem.isExpiringSoon(
  ...
  return expiration.before(midnightLater) ||
      expiration == midnightLater ||
      expiration == midnightToday
}
...
fun FridgeItem.getExpiringSoonMessage(now: Calendar): String {
  ...
  require(days >= 0)
  return if (days == 0L) "expires today"
  else {
    if (days < 7) {
