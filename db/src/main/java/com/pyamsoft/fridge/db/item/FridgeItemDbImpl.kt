...
@Singleton
internal class FridgeItemDbImpl
@Inject
internal constructor(
    @param:DbApi private val realAllItemsQueryDao: Cached<List<FridgeItem>>,
    @param:DbApi
    private val realItemsByEntryQueryDao:
        MultiCached1<FridgeEntry.Id, List<FridgeItem>, FridgeEntry.Id>,
    @param:DbApi
    private val realSameNameDifferentPresenceQueryDao:
        MultiCached2<
            FridgeItemDb.QuerySameNameDifferentPresenceKey, List<FridgeItem>, String, Presence>,
    @param:DbApi
    private val realSimilarNamedQueryDao:
        MultiCached2<
  ...
  override suspend fun querySimilarNamedItems(
      ...
      withContext(context = Dispatchers.IO) {
        ...
        return@withContext if (result.isEmpty()) emptyList()
        else {
          result.chunked(SIMILARITY_MAX_ITEM_COUNT).first().map { it.item }.toList()
        }
      }
