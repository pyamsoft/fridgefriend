...
internal data class RoomFridgeCategory
internal constructor(
    @JvmField @PrimaryKey @ColumnInfo(name = COLUMN_ID) val id: FridgeCategory.Id,
    @JvmField @ColumnInfo(name = COLUMN_NAME) val name: String,
    @JvmField @ColumnInfo(name = COLUMN_CREATED_TIME) val createdTime: Date,
    @JvmField @ColumnInfo(name = COLUMN_DEFAULT) val isDefaultCategory: Boolean,
    @JvmField
  ...
  companion object {
    ...
    @Ignore internal const val TABLE_NAME = "room_fridge_category_table"
    ...
    @Ignore internal const val COLUMN_DEFAULT = "default"

    @Ignore internal const val COLUMN_THUMBNAIL = "thumbnail"

    @Ignore
    @JvmStatic
    @CheckResult
    internal fun create(entry: FridgeCategory): RoomFridgeCategory {
      return if (entry is RoomFridgeCategory) entry
      else {
        RoomFridgeCategory(
            entry.id(), entry.name(), entry.createdTime(), entry.isDefault(), entry.thumbnail())
      }
    }
  }
}
